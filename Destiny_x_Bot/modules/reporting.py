...
from Destiny_x_Bot import LOGGER, DRAGONS, TIGERS, WOLVES, dispatcher
...
from Destiny_x_Bot.modules.sql import reporting_sql as sql
...
def report(update: Update, context: CallbackContext) -> str:
    ...
    if chat and message.reply_to_message and sql.chat_should_report(chat.id):
        ...
        if user.id == reported_user.id:
            message.reply_text("Uh yeah, Sure sure...nice joke?")
            return ""

        if user.id == bot.id:
            message.reply_text("Nice try. Noob!!!")
            return ""

        if reported_user.id in REPORT_IMMUNE_USERS:
            message.reply_text("Are you Comedy me? You reporting a disaster?")
            return ""

        if chat.username and chat.type == Chat.SUPERGROUP:

            reported = f"{mention_html(user.id, user.first_name)} reported {mention_html(reported_user.id, reported_user.first_name)} to the admins! Now they might beat his @$$."
