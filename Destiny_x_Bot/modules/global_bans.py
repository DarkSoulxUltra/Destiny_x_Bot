...
import Destiny_x_Bot.modules.sql.global_bans_sql as sql
from Destiny_x_Bot.modules.sql.users_sql import get_user_com_chats
...
from Destiny_x_Bot.modules.helper_funcs.misc import send_to_list
...
def gbanlist(update: Update, context: CallbackContext):
    ...
    if not banned_users:
        update.effective_message.reply_text(
            "There aren't any gbanned users! You're kinder than I expected, but lesser than my Maestro...",
        )
        return
...
def gbanstat(update: Update, context: CallbackContext):
    ...
    if len(args) > 0:
        if args[0].lower() in ["on", "yes"]:
            ...
            update.effective_message.reply_text(
                "Antispam is now enabled ✅ ! Hurray!!!"
                "I am now protecting this group with my might from potential remote threats!",
            )
