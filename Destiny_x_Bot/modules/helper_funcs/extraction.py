...
from Destiny_x_Bot import LOGGER
from Destiny_x_Bot.modules.users import get_user_id
...
def extract_user_and_text(
    ...
    if entities and ent and ent.offset == len(message.text) - len(text_to_parse):
        ...
        if not user_id:
            message.reply_text(
                "No idea who the F is this person. You'll be able to interact with them if "
                "you reply to that person's message instead, or forward one of that user's messages to me :).",
            )
            return None, None
